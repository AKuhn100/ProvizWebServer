...
# After your existing code for "select_file" etc., do something like:
DATA_DIR_20F = "summary_data_20F"

###############################################################
# B) Build a multi-figure layout for the 20F frustration data
###############################################################
...
def build_frustration_comparison_20F(filepath):
    ...
    to replicate the multi-subplot frustration figure in Bokeh form.
    
    :param filepath: Path to a 20F summary file.
    :return: A Bokeh layout (grid) containing the plots.
    """
    # ----------- 1) Read the data -----------
    rep1_df, rep2_df, evol_frust = read_frustration_file_20F(filepath)
    
    # Merge them for convenience so we can line up the same AlnIndex
    merged = rep1_df.merge(rep2_df, on='AlnIndex', suffixes=('_REP1','_REP2'))
    merged['EvolFrust'] = evol_frust
    
    # Filter out missing data for the main lines
    ...
    if filtered.empty:
        # If there's no valid data, return an empty layout
    ...
    # Bokeh ColumnDataSources for the smoothed lines
    source_main = ColumnDataSource(data=dict(
        x_rep1_exp = x_rep1_exp,
        y_rep1_exp = y_rep1_exp,
        x_rep2_exp = x_rep2_exp,
        y_rep2_exp = y_rep2_exp,
        x_evol = x_evol,
        y_evol = y_evol
    ))
    source_bf = ColumnDataSource(data=dict(
        x_rep1_bf = x_rep1_bf,
        y_rep1_bf = y_rep1_bf,
        x_rep2_bf = x_rep2_bf,
        y_rep2_bf = y_rep2_bf
    ...
    p_main = figure(
        ...
        title="Protein Frustration Comparison: REP1 vs REP2",
        tools="pan,box_zoom,wheel_zoom,reset,save",
        active_drag="box_zoom"
    )
    ...
    # We use line glyphs in Bokeh (solid color).
    # If you want gradient color similar to your Matplotlib example,
    # you must manually construct multi_line segments or use patches.
    
    # REP1 ExpFrust as a thicker line
    r_rep1_exp = p_main.line('x_rep1_exp', 'y_rep1_exp', source=source_main,
                             line_color="red", line_width=3,
                             legend_label="REP1 Experimental")
    # REP2 ExpFrust as a dashed line
    r_rep2_exp = p_main.line('x_rep2_exp', 'y_rep2_exp', source=source_main,
                             line_color="blue", line_width=3, line_dash="dashed",
                             legend_label="REP2 Experimental")
    # Evolutionary frustration as dotted line
    r_evol = p_main.line('x_evol', 'y_evol', source=source_main,
                         line_color="green", line_width=2, line_dash="dotdash",
                         legend_label="Evolutionary Frustration")

    p_main.legend.location = "top_left"
    p_main.legend.click_policy = "hide"

    # Optional: Add secondary structure “boxes” or “spans” for REP1 vs REP2
    # (Simplified approach with BoxAnnotation for each SS region):
    # You could parse rep1_df['SecondaryStructure'] and rep2_df['SecondaryStructure'],
    # find contiguous segments of 'A' or 'B', and overlay a tinted box.  
    # (Omitted here for brevity, but the logic is similar: for each region, 
    #  create a BoxAnnotation or Span from i_start to i_end with a certain fill_color.)

    # ----------- 4) Scatter subplots: B-Factor vs. frustration -----------
    # We'll create multiple small figures, each 300x300.
    # We want: 
    ...
    def create_scatter_figure(x_series, y_series, title, color):
        """
        Create a small scatter + rank regression line figure.
        """
        p = figure(width=300, height=300, 
                   ...
                   active_drag="box_zoom")
        # Drop NA
        df_local = pd.DataFrame({'x': x_series, 'y': y_series}).dropna()
        if len(df_local) < 2:
            return p  # no data
        ...
        # 0-1 normalize ranks to have a consistent scale
        rx_min, rx_max = df_local['rank_x'].min(), df_local['rank_x'].max()
        ry_min, ry_max = df_local['rank_y'].min(), df_local['rank_y'].max()
        df_local['rank_x_norm'] = (df_local['rank_x'] - rx_min) / (rx_max - rx_min + 1e-12)
        df_local['rank_y_norm'] = (df_local['rank_y'] - ry_min) / (ry_max - ry_min + 1e-12)

        source_scat = ColumnDataSource(df_local)
        p.circle('rank_x_norm', 'rank_y_norm', source=source_scat, size=6,
                 color=color, alpha=0.6)
        
        # Spearman correlation on original values
        rho, pval = spearmanr(df_local['x'], df_local['y'])
        # We do a linear fit on the rank_x_norm vs rank_y_norm
        slope, intercept, _, _, _ = linregress(df_local['rank_x_norm'], df_local['rank_y_norm'])
        x_line = np.linspace(0, 1, 50)
        y_line = slope * x_line + intercept
        p.line(x_line, y_line, line_dash='dashed', line_color='gray')
        
        # Show correlation in the title or as a label
        p.title.text = f"{title}\nSpearman ρ={rho:.3f}, p={pval:.1e}"
        p.xaxis.visible = False
        p.yaxis.visible = False
        return p

    p_s11 = create_scatter_figure(filtered['B_Factor_REP2'], filtered['ExpFrust_REP2'],
                  "REP2 ExpFrust vs. REP2 B-Factor", "blue")
    p_s12 = create_scatter_figure(filtered['B_Factor_REP2'], filtered['ExpFrust_REP1'],
                  "REP1 ExpFrust vs. REP2 B-Factor", "red")
    p_s13 = create_scatter_figure(filtered['B_Factor_REP2'], filtered['EvolFrust'],
                  "EvolFrust vs. REP2 B-Factor", "green")
    p_s21 = create_scatter_figure(filtered['B_Factor_REP1'], filtered['ExpFrust_REP2'],
                  "REP2 ExpFrust vs. REP1 B-Factor", "blue")
    p_s22 = create_scatter_figure(filtered['B_Factor_REP1'], filtered['ExpFrust_REP1'],
                  "REP1 ExpFrust vs. REP1 B-Factor", "red")
    p_s23 = create_scatter_figure(filtered['B_Factor_REP1'], filtered['EvolFrust'],
                  "EvolFrust vs. REP1 B-Factor", "green")
    
    # ----------- 5) Summary correlation dot chart -----------
    # We'll measure Spearman correlations for (B_Factor_REP1, B_Factor_REP2) vs 
    # each frustration metric: ExpFrust_REP1, ExpFrust_REP2, EvolFrust.
    
    combos = [
        ("REP1 B-Factor", filtered['B_Factor_REP1']),
        ("REP2 B-Factor", filtered['B_Factor_REP2'])
    ]
    metrics = [
        ("REP1 ExpFrust", filtered['ExpFrust_REP1'], "red"),
        ("REP2 ExpFrust", filtered['ExpFrust_REP2'], "blue"),
        ("EvolFrust",     filtered['EvolFrust'],      "green")
    ]
    data_summary = []
    for (bf_name, bf_col) in combos:
        for (met_name, met_col, color) in metrics:
            # Spearman correlation
            sub_df = pd.DataFrame({'bf': bf_col, 'met': met_col}).dropna()
            if len(sub_df) >= 2:
                rho, pval = spearmanr(sub_df['bf'], sub_df['met'])
            else:
                rho, pval = np.nan, np.nan
            data_summary.append((bf_name, met_name, rho, pval, color))
    
    p_summary = figure(width=400, height=300,
                       title="Summary of B-Factor Correlations",
                       tools="pan,box_zoom,reset,save")
    p_summary.x_range.range_padding = 0.3
    # X-axis are 3 metrics: "REP1 ExpFrust", "REP2 ExpFrust", "EvolFrust"
    # We'll place them at x=0,1,2 for convenience
    x_labels = ["REP1 ExpFrust", "REP2 ExpFrust", "EvolFrust"]
    p_summary.x_range.factors = x_labels  # categorical axis
    p_summary.yaxis.axis_label = "Spearman's ρ"
    # We place points for REP1 B-Factor and REP2 B-Factor slightly offset 
    # from each category's center.
    
    plot_data = {"x_cat":[],"rho":[],"color":[],"bf_group":[],"pval":[]}
    for (bf_name, met_name, rho, pval, color) in data_summary:
        plot_data["x_cat"].append(met_name)
        plot_data["rho"].append(rho)
        plot_data["color"].append(color)
        plot_data["bf_group"].append(bf_name)
        plot_data["pval"].append(pval)
    
    source_summary = ColumnDataSource(plot_data)
    
    # We can set up two offsets: -0.1 for REP1, +0.1 for REP2
    def bf_offset(bf):
        return -0.1 if "REP1" in bf else 0.1
    offset_vals = [bf_offset(g) for g in plot_data["bf_group"]]
    # We'll do circle glyphs for each point
    r_sum = p_summary.circle(x=dodge("x_cat", offset_vals, range=p_summary.x_range),
                             y="rho",
                             source=source_summary,
                             size=10, fill_color="color", line_color="black")
    # p-value < 0.05 => we highlight them somehow
    # For demonstration, we draw a square marker behind them if significant
    # or simply change the alpha. This is just an example:
    # We'll do a separate GlyphRenderer for squares:
    sig_mask = [p < 0.05 for p in plot_data["pval"]]
    source_sig = ColumnDataSource({
        "x_cat":[plot_data["x_cat"][i] for i,m in enumerate(sig_mask) if m],
        "rho":[plot_data["rho"][i] for i,m in enumerate(sig_mask) if m],
        "offset":[offset_vals[i] for i,m in enumerate(sig_mask) if m],
    })
    p_summary.square(x=dodge("x_cat","offset", range=p_summary.x_range),
                     y="rho",
                     size=20, fill_color=None, line_color="black", line_width=2,
                     source=source_sig)
    
    # Horizontal line at y=0
    p_summary.line(x_labels, [0,0,0], line_dash='dashed', color='gray')
    
    # ----------- 6) Normalized Smoothed B-Factor Plot -----------
    p_bf_norm = figure(width=800, height=250,
                       title="Normalized Smoothed B-Factor: REP1 vs REP2",
                       tools="pan,box_zoom,wheel_zoom,reset,save",
                       active_drag="box_zoom")
    p_bf_norm.xaxis.axis_label = "Residue Number"
    p_bf_norm.yaxis.axis_label = "Normalized B-Factor"
    
    # Min-max normalize the smoothed B-Factors
    rep1_bf_norm = min_max_normalize(pd.Series(y_rep1_bf))
    rep2_bf_norm = min_max_normalize(pd.Series(y_rep2_bf))

    source_bf_norm = ColumnDataSource(data=dict(
        x_rep1_bf = x_rep1_bf,
        r1_bf_norm = rep1_bf_norm,
        x_rep2_bf = x_rep2_bf,
        r2_bf_norm = rep2_bf_norm
    ))
    p_bf_norm.line('x_rep1_bf', 'r1_bf_norm', source=source_bf_norm,
                   line_color="blue", line_width=2, legend_label="REP1 B-Factor")
    p_bf_norm.line('x_rep2_bf', 'r2_bf_norm', source=source_bf_norm,
                   line_color="orange", line_width=2, legend_label="REP2 B-Factor")
    p_bf_norm.legend.location = "top_right"

    # ----------- 7) B-Factor rank scatter (REP1 vs REP2) -----------
    p_bf_rank = figure(width=800, height=250,
    ...
    # Rank the raw B-Factors
    bf1_rank = filtered['B_Factor_REP1'].rank()
    bf2_rank = filtered['B_Factor_REP2'].rank()
    source_rank = ColumnDataSource(data=dict(
        bf1_rank = bf1_rank,
        bf2_rank = bf2_rank
    ))
    p_bf_rank.scatter('bf1_rank', 'bf2_rank', source=source_rank,
                      marker="diamond", size=8, color="purple", alpha=0.6)
    ...
    y_line = slope * x_line + intercept
    p_bf_rank.line(x_line, y_line, line_color="gray", line_dash="dashed")
    p_bf_rank.title.text += f"\nSpearman ρ={rho_bf:.3f}, p={pval_bf:.1e}"

    # ----------- 8) Assemble grid -----------
    # We’ll replicate a similar layout to your 7-row approach. 
    # In Bokeh, you typically nest or use gridplot. 
    # Here is a possible arrangement:
    
    scatter_grid_left = column(p_s11, p_s12, p_s13)
    scatter_grid_right = column(p_s21, p_s22, p_s23)
    scatter_grid = row(scatter_grid_left, scatter_grid_right)
    
    # Now the final layout
    # row1: main plot
    # row2: scatter subplots
    # row3: summary correlation + possibly empty placeholder
    # row4: normalized B-Factor
    # row5: B-Factor rank scatter
    grid = column(
        p_main,
        scatter_grid,
        row(p_summary),
...
# Placeholder for the layout that displays the 20F multi-figure
...
if files_20F:
    layout_20F_display.children = [build_frustration_comparison_20F(os.path.join(DATA_DIR_20F, files_20F[0]))]
...
# 
# This section contains:
#  - read_frustration_file_20F
#  - lowess_smoothing (if needed)
#  - build_frustration_comparison_20F
#  - etc.
###############################################################################

def read_frustration_file_20F(filepath):
    """
    Read the 20F summary file, returning three structures:
    rep1_df, rep2_df, and evol_frust.
...
visualization_section_20F = column(
    ...
    select_file_20F,
    layout_20F_display,  # the column we update on selection
    sizing_mode='stretch_width'
)
