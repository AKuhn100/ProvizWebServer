...
FRUSTRATION_COLORS = {
    ...
    "Spearman_Diff": Category10[10][3]  # Orange (Added for Spearman_Diff)
}
...
# (H) Spearman Rho per Protein and Frustration Metric (Plot Three)
# Melt data_proviz for the third plot
data_long_corr = data_proviz.melt(
    id_vars=['Protein'],
    value_vars=['Spearman_ExpFrust', 'Spearman_AFFrust', 'Spearman_EvolFrust', 'Spearman_Diff'],  # Included 'Spearman_Diff'
...
# (F) NEW: Bar Plot with Mean, SD, and Spearman_Diff
def create_bar_plot_with_sd_and_diff(data_proviz):
    """
    Creates a bar chart displaying the mean Spearman correlation for each frustration metric,
    including Spearman_Diff, with error bars representing the standard deviation.
    Adjusts the y-axis range to ensure whiskers are fully visible.
    """
    # Compute mean and standard deviation of Spearman Rho per metric, including Spearman_Diff
    spearman_columns = ['Spearman_ExpFrust', 'Spearman_AFFrust', 'Spearman_EvolFrust', 'Spearman_Diff']
    stats_corrs = data_proviz[spearman_columns].agg(['mean', 'std']).transpose().reset_index()
    stats_corrs.rename(columns={
        'index': 'Metric',
        'mean': 'Mean_Spearman_Rho',
...
visualization_section = column(
    ...
    scatter_row,
    unity_container,
    create_bar_plot_with_sd_and_diff(data_proviz),  # Integrated Bar Plot with Spearman_Diff
    create_bar_plot_with_sd(data_proviz),  # Original Bar Plot (optional)
    create_bar_plot_with_sd(data_proviz),  # Duplicate if needed
    sizing_mode='stretch_width',
    spacing=20,
